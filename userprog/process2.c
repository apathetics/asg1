...
{
  char *save_ptr, *cmd_copy1; // copy1 (of cmd_line) is to process file name (strtok will change it)
  char *cmd_copy2; // copy2 (of cmd_line) is sent to the new thread, not edited by anything else
  ...
  cmd_copy2 = palloc_get_page (0); // should this be 0 or something else???
  ...
  // isolate the first argument (i.e. the file to open) 
  // NOTE: this code damages cmd_copy1!!!
...
start_process (void *cmd_line)
{
  ...
  // perform argument parsing, pushing each argment on to argv
  char *token, *string_copy, *argv[50];
  int argc = 0;
  for (token = strtok_r (cmd_line, " ", &string_copy); token != NULL;
       token = strtok_r (NULL, " ", &string_copy))
  {
    if(DBP) printf ("arg %d: '%s'\n", argc, token);
    argv[argc++] = token;
  }
  argv[argc] = NULL;
  ...
  if (kpage != NULL) 
	...
	for(i = argc - 1; i >= 0; i--)
	   ...
	   //push argv[i] onto stack, and change esp
           ...
           esp_prev = *esp;
           *esp -= arg_size;
           strlcpy(*esp, argv[i], arg_size); // should we decrement by 1?      
           
           // store *esp pointer for next step
	   arg_pointers[i] = *esp;
        ...
        *esp = word_align(*esp, sizeof(void*));
        ...
        esp_prev = *esp;
        *esp -= sizeof(char *); // should be 4
        memset(*esp, 0 /*argv[argc]*/, sizeof(char*));
  	...
  	for(i = argc - 1; i >= 0; i--)
 	{
 	    //push argv[i] onto stack, and change esp
           esp_prev = *esp;
           *esp -= sizeof(char *); // should be 4
           memcpy(*esp, &arg_pointers[i], sizeof(char*)); // should we decrement by 1?
